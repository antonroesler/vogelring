...
class S3PickleMigrator:
    ...
    def __init__(self, dry_run: bool = False, user: str = DEFAULT_USER, json_file: Optional[str] = None):
    ...
    def convert_pickle_item_to_sighting(self, item: Any) -> Optional[Sighting]:
        ...
        try:
            ...
            logger.error(f"Item data: {json.dumps(item, ensure_ascii=False, default=str)}")
            return None
    ...
    def validate_sighting_data_integrity(self):
        ...
        try:
            with get_db_session() as session:
                ...
                orphaned_sightings = session.execute(sql_text("""
                ...
                duplicate_excel_ids = session.execute(sql_text("""
                ...
                invalid_coords = session.execute(sql_text("""
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description='Migrate sighting data from S3 pickle files or local JSON to PostgreSQL')
    ...
    parser.add_argument('--json-file', default=None, help='Path to local JSON file with sightings list')
    ...
    migrator = S3PickleMigrator(dry_run=dry_run, user=args.user, json_file=args.json_file)
