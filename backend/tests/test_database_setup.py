...
class DatabaseTestUtils:
    ...
    def verify_database_integrity(session):
        ...
        duplicate_rings = session.execute(
            text("""
        ...
        if duplicate_rings:
            integrity_issues.append(
                f"Duplicate rings in ringings table: {duplicate_rings}"
            )

        # Check for duplicate ring numbers in family_tree_entries
        duplicate_family_rings = session.execute(
            text("""
            SELECT ring, COUNT(*) as count 
        ...
        if duplicate_family_rings:
            integrity_issues.append(
                f"Duplicate rings in family_tree_entries table: {duplicate_family_rings}"
            )
        ...
        if orphaned_sightings and orphaned_sightings[0] > 0:
            integrity_issues.append(
                f"Orphaned sightings (ring not in ringings): {orphaned_sightings[0]}"
            )

        return {"valid": len(integrity_issues) == 0, "issues": integrity_issues}
...
class TestDatabasePerformanceUtils:
    ...
    def test_query_performance_with_indexes(self):
        ...
        results = (
            session.query(Sighting).filter(Sighting.species == "Larus ridibundus").all()
        )
