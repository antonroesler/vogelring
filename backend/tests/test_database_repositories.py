...
from src.database.repositories import SightingRepository, RingingRepository, FamilyTreeRepository
...
class TestSightingRepository:
    ...
    def test_get_enriched_sightings(self, test_db, create_test_sighting, create_test_ringing):
...
class TestFamilyTreeRepository:
    """Test family tree repository operations"""
    
    def test_create_family_tree_entry(self, test_db, sample_family_tree_data):
        """Test creating a family tree entry"""
        repo = FamilyTreeRepository(test_db)
        entry = repo.create(**sample_family_tree_data)
        
        assert entry.id is not None
        assert entry.ring == "TEST001"
        assert entry.partners is not None
        assert entry.children is not None
    
    def test_get_by_ring(self, test_db, create_test_family_tree):
        """Test getting family tree entry by ring"""
        repo = FamilyTreeRepository(test_db)
        entry = repo.get_by_ring(create_test_family_tree.ring)
        
        assert entry is not None
        assert entry.ring == create_test_family_tree.ring
    
    def test_upsert_family_entry_create(self, test_db):
        """Test upserting a new family tree entry"""
        repo = FamilyTreeRepository(test_db)
        partners = [{"ring": "PARTNER001", "year": 2023, "confirmed": True}]
        children = [{"ring": "CHILD001", "year": 2023, "confirmed": True}]
        
        entry = repo.upsert_family_entry("NEWTEST001", partners=partners, children=children)
        
        assert entry.ring == "NEWTEST001"
        assert entry.partners == partners
        assert entry.children == children
    
    def test_upsert_family_entry_update(self, test_db, create_test_family_tree):
        """Test upserting an existing family tree entry"""
        repo = FamilyTreeRepository(test_db)
        ring = create_test_family_tree.ring
        
        new_partners = [{"ring": "NEWPARTNER", "year": 2024, "confirmed": True}]
        updated_entry = repo.upsert_family_entry(ring, partners=new_partners)
        
        assert updated_entry.ring == ring
        assert updated_entry.partners == new_partners
        # Should be the same record
        assert updated_entry.id == create_test_family_tree.id
    
    def test_get_children_of(self, test_db, create_test_family_tree):
        """Test getting children of a bird"""
        repo = FamilyTreeRepository(test_db)
        children = repo.get_children_of(create_test_family_tree.ring)
        
        assert len(children) == 1
        assert "TEST003" in children
    
    def test_get_partners_of(self, test_db, create_test_family_tree):
        """Test getting partners of a bird"""
        repo = FamilyTreeRepository(test_db)
        partners = repo.get_partners_of(create_test_family_tree.ring)
        
        assert len(partners) == 1
        assert "TEST002" in partners
    
    def test_get_family_tree(self, test_db, create_test_family_tree):
        """Test getting complete family tree"""
        repo = FamilyTreeRepository(test_db)
        family_tree = repo.get_family_tree(create_test_family_tree.ring, depth=1)
        
        assert family_tree['ring'] == create_test_family_tree.ring
        assert 'partners' in family_tree
        assert 'children' in family_tree
        assert 'parents' in family_tree
...
class TestDatabaseIntegrity:
    ...
    def test_cascade_relationships(self, test_db, create_test_sighting, create_test_ringing):
        ...
        sighting_with_ringing = (test_db.query(Sighting)
