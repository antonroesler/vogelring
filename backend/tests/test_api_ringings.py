...
class TestRingingsAPI:
    ...
    def test_get_autocomplete_suggestions_with_limit(self, client, multiple_test_ringings):
    ...
    def test_date_range_filter(self, client, multiple_test_ringings):
        ...
        response = client.get(f"/api/ringings?start_date={start_date}&end_date={end_date}")
        assert response.status_code == 200
        data = response.json()
        ...
        response = client.get(f"/api/ringings?start_date={future_start}&end_date={future_end}")
        assert response.status_code == 200
        data = response.json()
