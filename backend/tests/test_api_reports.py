...
class TestReportsAPI:
    ...
    def test_create_shareable_report_success(
        self, mock_get_s3_client, client, multiple_test_sightings
    ):
    ...
    @patch("src.api.routers.reports.get_s3_client")
    def test_create_shareable_report_with_filters(
        self, mock_get_s3_client, client, multiple_test_sightings
    ):
    ...
    def test_create_shareable_report_no_credentials(self, mock_get_s3_client, client):
        ...
        report_request = {"title": "Test Report", "include_sightings": True}
    ...
    def test_create_shareable_report_validation_error(self, client):
        ...
        invalid_request = {"include_sightings": True}
    ...
    def test_get_presigned_url_success(self, mock_get_s3_client, client):
        ...
        mock_s3.generate_presigned_url.return_value = (
            "https://s3.amazonaws.com/new-presigned-url"
        )

        response = client.get(
            "/api/report/presigned-url?s3_key=reports/test_report.json&expiry_hours=24"
        )
        assert response.status_code == 200
        ...
        mock_s3.head_object.assert_called_once_with(
            Bucket="vogelring-data", Key="reports/test_report.json"
        )
        mock_s3.generate_presigned_url.assert_called_once()
    ...
    def test_get_presigned_url_not_found(self, mock_get_s3_client, client):
        ...
        response = client.get(
            "/api/report/presigned-url?s3_key=reports/nonexistent.json"
        )
    ...
    def test_list_reports_success(self, mock_get_s3_client, client):
        ...
        mock_s3.head_object.side_effect = [
            {"Metadata": {"title": "Report 1", "created_at": "2023-06-01T12:00:00Z"}},
            {"Metadata": {"title": "Report 2", "created_at": "2023-06-02T12:00:00Z"}},
        ]
    ...
    def test_delete_report_success(self, mock_get_s3_client, client):
        ...
        # Verify S3 interactions
        mock_s3.head_object.assert_called_once_with(
            Bucket="vogelring-data", Key="reports/test_report_123.json"
        )
    ...
    def test_report_content_structure(
        self, mock_get_s3_client, client, create_test_sighting, create_test_ringing
    ):
