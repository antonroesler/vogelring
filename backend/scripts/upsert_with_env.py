...
assert ORG_ID, "ORG_ID is not set"
        ...
        try:
            with open(csv_file, "r", encoding="utf-8") as f:
                ...
                for row_num, row in enumerate(
                    ...
                    try:
                        # Check if ringing already exists
                        existing = service.get_ringing_by_ring(
                            ring=str(ringing.ring), org_id=ORG_ID
                        )
                        ...
                        if existing:
                            ...
                            if existing.comment != ringing.comment:
                                success = upsert_ringing_with_comment_only(
                                    service,
                                    ring=str(ringing.ring),
                                    comment=str(ringing.comment),
                                    org_id=ORG_ID,
                                )
                                if success:
                            ...
                            service.upsert_ringing(ringing_data=ringing_data)
