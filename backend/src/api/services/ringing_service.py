...
class RingingService:
    ...
    def get_ringing_by_ring(self, ring: str, org_id: str) -> Optional[RingingDB]:
        """Get ringing by ring number"""
        return self.repository.get_by_ring(ring, org_id)

    def get_all_ringings(
    ...
    def get_ringings_by_species(self, species: str, org_id: str) -> List[RingingDB]:
        """Get all ringings for a specific species"""
        return self.repository.get_by_species(species, org_id)

    def get_ringings_by_ringer(self, ringer: str, org_id: str) -> List[RingingDB]:
        """Get all ringings by a specific ringer"""
        return self.repository.get_by_ringer(ringer, org_id)

    def get_ringings_by_place(self, place: str, org_id: str) -> List[RingingDB]:
        """Get all ringings at a specific place"""
        return self.repository.get_by_place(place, org_id)

    def get_ringings_by_date_range(
    ...
    def search_ringings(self, filters: Dict[str, Any], org_id: str) -> List[RingingDB]:
        """Search ringings with multiple filters"""
        return self.repository.search_ringings(filters, org_id)

    def upsert_ringing(self, org_id: str, ringing_data: Dict[str, Any]) -> RingingDB:
        ...
        try:
            ...
            ringing = self.repository.upsert_ringing(ring, org_id, **ring_data)
    ...
    def delete_ringing(self, ring: str, org_id: str) -> bool:
        """Delete a ringing record by ring number"""
        try:
            ringing = self.repository.get_by_ring(ring, org_id)
            if not ringing:
                # Return True even if ring doesn't exist (matches original behavior)
                logger.info(
                    f"Ring {ring} not found, but delete operation considered successful"
                )
                return True

            result = self.repository.delete(ringing.id, org_id)
            if result:
    ...
    def get_ringings_count(self, org_id: str) -> int:
        """Get total count of ringings"""
        return self.db.query(RingingDB).filter(RingingDB.org_id == org_id).count()
    ...
    def get_entry_list_ringings(
        ...
        return self.repository.get_entry_list_ringings(
            filters, org_id=org_id, limit=limit, offset=offset
        )
    ...
    def get_entry_list_ringings_count(
        ...
        return self.repository.get_entry_list_ringings_count(filters, org_id=org_id)
