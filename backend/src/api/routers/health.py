...
async def detailed_health_check(db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    try:
        ...
        if system_stats["cpu_percent"] > 80:
            health_data["checks"]["system"]["warnings"] = health_data["checks"]["system"].get("warnings", []) + ["High CPU usage"]
        if system_stats["disk_percent"] > 90:
            health_data["checks"]["system"]["warnings"] = health_data["checks"]["system"].get("warnings", []) + ["High disk usage"]
