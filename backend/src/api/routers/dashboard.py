...
async def get_dashboard(
    days: int = Query(30, ge=1, le=365, description="Number of days to include in recent activity"),
    ...
    top_species = (db.query(Sighting.species, func.count(Sighting.species).label('count'))
                  .filter(Sighting.date >= start_date)
                  .filter(Sighting.date <= end_date)
                  .filter(Sighting.species.isnot(None))
    ...
    top_locations = (db.query(Sighting.place, func.count(Sighting.place).label('count'))
                    .filter(Sighting.date >= start_date)
                    .filter(Sighting.date <= end_date)
    ...
    return {
        ...
        "top_locations": [
            {"location": row.place, "count": row.count}
...
async def get_dashboard_summary(db: Session = Depends(get_db)):
    ...
    # Get date ranges
    sighting_dates = (db.query(func.min(Sighting.date), func.max(Sighting.date))
                     .filter(Sighting.date.isnot(None))
                     .first())
    
    ringing_dates = (db.query(func.min(Ringing.date), func.max(Ringing.date))
                    .first())
    
    return {
        "totals": {
            "sightings": total_sightings,
            "ringings": total_ringings
        },
        "date_ranges": {
            "sightings": {
                "earliest": sighting_dates[0].isoformat() if sighting_dates[0] else None,
